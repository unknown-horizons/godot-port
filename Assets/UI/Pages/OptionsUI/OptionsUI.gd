...
func _ready() -> void:
	...
	settings["WindowMode"].item_selected.connect(_on_WindowMode_item_selected)
	...
	settings["ScreenResolution"].item_selected.connect(_on_ScreenResolution_item_selected)

	# Audio parameters
	settings["MasterVolume"].value = Config.master_volume
	settings["MasterVolume"].value_changed.connect(_on_MasterVolume_value_changed)

	settings["MusicVolume"].value = Config.music_volume
	settings["MusicVolume"].value_changed.connect(_on_MusicVolume_value_changed)

	settings["EffectsVolume"].value = Config.effects_volume
	settings["EffectsVolume"].value_changed.connect(_on_EffectsVolume_value_changed)

	settings["VoiceVolume"].value = Config.voice_volume
	settings["VoiceVolume"].value_changed.connect(_on_VoiceVolume_value_changed)
