...
func get_tile_item(tile_pos: Vector2) -> int:
	return get_cell_item(tile_pos.x, 0, tile_pos.y)
...
func get_tile_item_orientation(tile_pos: Vector2) -> int:
	return get_cell_item_orientation(tile_pos.x, 0, tile_pos.y)
...
func tilemap_to_world(tile_pos: Vector2) -> Vector2:
	return Utils.map_3_to_2(map_to_world(tile_pos.x, 0, tile_pos.y))

# Returns the coordinates of the tile cell containing the given point.
func world_to_tilemap(pos: Vector2) -> Vector2:
	var cell_pos := world_to_map(Utils.map_2_to_3(pos))
