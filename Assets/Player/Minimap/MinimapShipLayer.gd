...
func _draw():
	...
	for player_ship in player_ships:
		draw_set_transform(player_ship, 
			...
			ship_icon_scale)
		draw_texture(player_ship_icon, player_ship_icon.get_size() * ship_icon_scale / -2)
...
func get_ships():
		...
		for player_ship in player_ships_node.get_children():
			if player_ship is Ship:
				player_ships.append(minimap.world_to_minimap_position(player_ship.global_transform.origin))
		...
		#		trader_ships.append(minimap.world_to_minimap_position(trader_ship.global_transform.origin))
		...
		#		pirate_ships.append(minimap.world_to_minimap_position(pirate_ship.global_transform.origin))
